<template>
    <div>
        <section id="footer" class="fixed left-0 right-0 bottom-0 flex w-full h-10 z-50 bg-white text-gray-400">
            ...
            <!-- 搜索 -->
            <section @click="gotoAddress({path: `/search/`})" class="flex flex-col flex-1 justify-center items-center">
                <svg class="w-4 h-4 fill-current">
...
export default {
    ...
    setup() {
        ...
        onMounted(() => {
            state.geohash = getStore('geohash')
        })
        // console.log(store.state.geohash)
        const gotoAddress = (path) => {
            if(path.path=='/search/'){
                path.path = path.path + getStore('geohash')
            }

            // path.query.geohash = getStore('geohash')
            console.log('本次Footer跳转:',path)
            router.push(path)
        }
        return {
            ...toRefs(state),
