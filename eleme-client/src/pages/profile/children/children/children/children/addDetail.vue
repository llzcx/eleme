...
export default {
    ...
    setup() {
        ...
        const searchAddress = () => {
            getTipsByKey(state.inputVal).then(resp=>{
                if(resp.data.tips && resp.data.tips.length == 0) {
                    return;
                }
                state.addressList = resp.data.tips
                state.noData = false
            })
        }
        const chooseAddress = (item) => {
            ...
            useMutation.SAVE_ADDDETAIL[0](item)
            router.go(-1)
