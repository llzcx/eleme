    ...
    <div>
        ...
        <section class="flex bg-blue-400 mt-8 p-2">
            <div v-if="userInfo&&userInfo.id">
                <van-image width="2.5rem" height="2.5rem" round :src="userInfo.avatar" />
            </div>
            <div v-else>
            ...
            <router-link :to="userInfo&&userInfo.id ? '/profile/info' : '/login'" class="flex-1 flex justify-between items-center ml-2 text-white">
...
export default {
    ...
    setup() {
        ...
        watch(() => store.state.userInfo, (val) => {
           console.log(new Date().getDate+ ":profile监听到改变");
            initData()
