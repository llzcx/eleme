                ...
                <li v-for="(item, index) in deliverable" :key="index" class="flex items-center ml-2 py-2 border-b" @click="chooseAddress(item, index)">
                    ...
                    <section>
                        ...
                        <div class="ml-2">
                            ...
                            <span class="text-xxs text-gray-400">{{item.specificAddress}}</span>
                        </div>
                    </section>
                </li>
            </ul>
    ...
    setup() {
        ...
        const initData = async() => {
            ...
            if(store.state.userInfo && store.state.userInfo.id) {
                const addressList = await getAddressListByCId(store.state.userInfo.id)
                state.addressList = addressList.data.data;
                // 将地址列表分为有效地址和无效地址
