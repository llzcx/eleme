<template>
    <div class="pt-8">
        ...
        <head-top :head-title="city_name" :go-back="true">
            <template #changeCity>
                <router-link to="/home" class="absolute top-1/2 right-2 transform -translate-y-1/2 text-white text-xxs">返回</router-link>
            </template>
        </head-top>
        <!-- form表单 禁止默认事件触发 -->
        ...
        <ul class="bg-gray-50 border-t-2 border-b-2">
            <li v-for="(item, index) in placeList" :key="index" @click="nextPage(index, item)" class="mb-1">
...
import {getTipsByKey} from '@/api8080/gaode'
export default {
    name: 'City',
    components: {
        HeadTop
    ...
    setup() {
        ...
        const initCity = async() => {
            ...
            // //在store里面存储
            // useMutation.RECORD_CITY_ID[0](state.city_id)
            // useMutation.RECORD_CITY_NAME[0](state.city_name)
            // //在本地存储 持久化
            // setStore('city_id',state.city_id);
            // setStore('city_name',state.city_name);
        ...
        const posPlace = () => {
            if(state.inputValue) {
                getTipsByKey(state.inputValue).then((res) => {
                    console.log(res)
                    state.showHistory = false
                    state.placeList = res.data.tips
        ...
        const nextPage = (index,item) => {
            const geohash = item.location

            //将geohash存储到localstorage
            setStore('geohash',geohash)
            //将geohash存储到vuex当中
            useMutation.SAVE_GEOHASH[0](geohash)
            useMutation.SAVE_POSITION[0](geohash)
