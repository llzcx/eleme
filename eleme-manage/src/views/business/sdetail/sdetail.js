...
import {getTipsByKey,regeo} from '@/api/business/gaode.js'
import {
  getResturants,
  getResturantsCount,
  foodCategory,
...
export default {
  ...
  methods: {
    ...
    async querySearchAsync(queryString, cb) {
      if (queryString) {
        try {
          const resp = await getTipsByKey(queryString)
          if (resp.data.tips instanceof Array) {
            resp.data.tips.map(item => {
              item.value = item.district + " " + item.name
              return item
            })
            cb(resp.data.tips)
          }
    ...
    addressSelect(vale) {
      console.log("select!",vale)
      let geohash = vale.location
      let addressName = vale.district +" "+ vale.address + " " + vale.name
      let longitude = geohash.split(',')[0]
      let latitude = geohash.split(',')[1]
      this.selectTable.address = addressName
      this.selectTable.longitude = longitude
      this.selectTable.latitude = latitude
