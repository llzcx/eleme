...
export default {
  ...
  created() {
    this.listQuery.shopId =  store.getters.shopInfo.id
    this.restaurant_id =  store.getters.shopInfo.id
    this.initData()
  ...
  methods: {
    ...
    async fetchData() {
      if( store.getters.shopInfo == null) {
        console.log("拉去失败[没有登录]")
        return;
      }
      //拉取商家食品分类
      let resp = await getCategoryByShopId(this.listQuery.shopId)
      console.log(resp)
